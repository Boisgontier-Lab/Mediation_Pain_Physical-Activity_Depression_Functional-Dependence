...
CLSA.FU2 <- CLSA.FU2 %>%
  dplyr::select(entity_id, startdate_COF2, startdate_TRF2, ADL_ABLDR_COF2, ADL_HPDR_COF2, ADL_UNDR_COF2, ADL_ABLFD_COF2, 
  ...
  dplyr::select(entity_id, startdate_FU2, ADL_ABLDR ,ADL_HPDR  ,ADL_UNDR  ,ADL_ABLFD  ,ADL_HPFD  ,
...
CLSA.FU2 <- CLSA.FU2 %>%
  mutate(ADL_NBRMIS = rowSums(dplyr::select(., all_of(existing_vars_to_sum)), na.rm = TRUE))
...
CLSA.FU2 <- CLSA.FU2%>%
  mutate(ADL_TDSUM = rowSums(dplyr::select(., all_of(existing_vars_to_sum)), na.rm = TRUE))
...
# selecting only three specific columns of follow-up dataset + Create binary variable excluding meal prep
CLSA.FU2 <- CLSA.FU2 %>%
  mutate(Binary_FU2_IADL_exclMeal = case_when(
    ADL_DCLST %in% c(0, 1) ~ 0,
    ADL_DCLST %in% c(2, 3, 4) ~ 1,
    TRUE ~ NA_integer_
  )) %>%
  dplyr::select(entity_id, ADL_DCLS_F2, ADL_DCLST, Binary_FU2_IADL_exclMeal, startdate_FU2)
...
## 2) With variable of functional dependence that excludes meals
m1.1.exclMeal <- process(data = CLSA.baseline.arthritis,
                         y = "Binary_FU2_IADL_exclMeal",  # new outcome that *excludes* meal prep
                         x = "HUP_FREE_MCQ",
                         m = c("log_trans_DEP_CESD10_c","PASE_score_c"),
                         cov = c("sex_numeric", "AGE_NMBR_c", "Binary_IADL.ADL_baseline"),
                         model = 6)

## 3) osteoarthritis - no/mild vs moderate/sever (Binary_FU2_IADL.1)
# Serial Mediation
...
## 4) rheumatoid arthritis no/mild vs moderate/sever (Binary_FU2_IADL.1)
# Serial Mediation
...
## 5) Mediation approach based on Structural Equation Modeling (SEM) (lavaan package)

model <- '
  # Paths from predictor to mediators
  log_trans_DEP_CESD10_c ~ a1*HUP_FREE_MCQ + c1*sex_numeric + c2*AGE_NMBR_c + c3*Binary_IADL.ADL_baseline
  PASE_score_c           ~ a2*log_trans_DEP_CESD10_c + a3*HUP_FREE_MCQ + c4*sex_numeric + c5*AGE_NMBR_c + c6*Binary_IADL.ADL_baseline

  # Path from mediators and predictor to binary outcome
  Binary_FU2_IADL.1      ~ b1*log_trans_DEP_CESD10_c + b2*PASE_score_c + c_prime*HUP_FREE_MCQ + c7*sex_numeric + c8*AGE_NMBR_c + c9*Binary_IADL.ADL_baseline

  # Indirect effects
  ind1 := a1 * b1                     # HUP_FREE_MCQ → DEP → Outcome
  ind2 := a3 * b2                     # HUP_FREE_MCQ → PASE → Outcome
  ind3 := a1 * a2 * b2                # HUP_FREE_MCQ → DEP → PASE → Outcome
  total_ind := ind1 + ind2 + ind3    # Total indirect effect
  total := c_prime + total_ind       # Total effect
'

fit <- sem(
  model,
  data = CLSA.baseline.arthritis,
  ordered = "Binary_FU2_IADL.1",
  estimator = "WLSMV"
)
...
## 6) Mediation approach based on causal inference (mediation package)

# Make sure mediators are numeric
CLSA.baseline.arthritis$log_trans_DEP_CESD10_c <- as.numeric(CLSA.baseline.arthritis$log_trans_DEP_CESD10_c)
CLSA.baseline.arthritis$PASE_score_c <- as.numeric(CLSA.baseline.arthritis$PASE_score_c)

# Step 1: Fit mediator models
# M1: Depressive symptoms as mediator
model.M1 <- lm(log_trans_DEP_CESD10_c ~ HUP_FREE_MCQ + sex_numeric + AGE_NMBR_c + Binary_IADL.ADL_baseline,
               data = CLSA.baseline.arthritis)

# M2: Physical activity as mediator
model.M2 <- lm(PASE_score_c ~ HUP_FREE_MCQ + sex_numeric + AGE_NMBR_c + Binary_IADL.ADL_baseline,
               data = CLSA.baseline.arthritis)

# Step 2: Fit outcome model (includes both mediators)
model.Y <- glm(Binary_FU2_IADL.1 ~ HUP_FREE_MCQ + log_trans_DEP_CESD10_c + PASE_score_c +
                 sex_numeric + AGE_NMBR_c + Binary_IADL.ADL_baseline,
               family = binomial(link = "logit"),
               data = CLSA.baseline.arthritis)


# Step 3: Conduct mediation analyses separately
# Mediation via depressive symptoms
med.out.M1 <- mediate(model.M1, model.Y,
                      treat = "HUP_FREE_MCQ",
                      mediator = "log_trans_DEP_CESD10_c",
                      boot = TRUE,
                      sims = 5000)

# Mediation via physical activity
med.out.M2 <- mediate(model.M2, model.Y,
                      treat = "HUP_FREE_MCQ",
                      mediator = "PASE_score_c",
                      boot = TRUE,
                      sims = 5000)
...
digraph sem_path {
  ...
  graph [layout = dot, rankdir = LR, splines = true, ranksep = 1.4]

  # Nodes
  X  [label = 'Presence of\\nPain', shape = ellipse, style = filled, fillcolor = lightblue]
  M1 [label = 'Depressive\\nSymptoms', shape = ellipse, style = filled, fillcolor = lightyellow]
  M2 [label = 'Physical\\nActivity', shape = ellipse, style = filled, fillcolor = lightyellow]
  Y  [label = 'Functional\\nDependence', shape = ellipse, style = filled, fillcolor = lightblue]
  Note [style = invis]  
  ...
  # Direct effect (c′)
  X  -> Y  [label = 'c′', color = red, fontcolor = red]

  # Total effect (c)
  X  -> Y  [label = 'c', style = dashed, color = gray, fontcolor = gray, constraint = false]

  # Indirect effect summary note
  X -> Note [style = invis]
...
digraph sem_path {
  ...
  X  -> M2 [label = 'b = -.083***']
  M1 -> M2 [label = 'b: -.030***']
  M1 -> Y  [label = 'log OR = .358***']
  M2 -> Y  [label = 'log OR = -0.598***']

  # Direct effect (c′)
  X  -> Y  [label = 'log OR = .607***', color = red, fontcolor = red]

  # Total effect (c)
  X  -> Y  [label = 'log OR = .791***', style = dashed, color = gray, fontcolor = gray, constraint = false]
...
data <- data.frame(
  Group = rep(c("Total", "Depressive Symptoms", "Physical Activity", "Serial Pathway"), each = 5),
  Condition = rep(c("All Arthritis 1", "All Arthritis 2","All Arthritis 3", "Osteoarthritis", "Rheumatoid Arthritis"), times = 4),
  Estimate = c(0.184, 0.152, 0.154, 0.203, 0.250,  # Total indirect effect
               0.128, 0.120, 0.112, 0.139, 0.185,  # Depressive symptoms
               0.050, 0.028, 0.038, 0.055, 0.055,  # Physical activity
               0.006, 0.004, 0.005, 0.009, 0.010), # Serial pathway
  Lower_CI = c(0.110, 0.117, 0.072, 0.113, 0.049, 
               0.066, 0.089, 0.036, 0.062, 0.013, 
               0.019, 0.013, 0.009, 0.016, -0.089, 
               0.001, 0.001, 0.001, 0.002, -0.013),
  Upper_CI = c(0.267, 0.189, 0.247, 0.307, 0.574, 
               0.200, 0.154, 0.199, 0.227, 0.453, 
               0.090, 0.046, 0.079, 0.109, 0.226, 
               0.013, 0.007, 0.011, 0.019, 0.038)
)
...
## Illustration of the mediation model based on the lavaan Package (SEM)
grViz("
digraph sem_path {
  ...
  # Nodes
  X  [label = 'Presence of\\nPain\\n(X)', shape = ellipse, style = filled, fillcolor = lightblue]
  M1 [label = 'Depressive\\nSymptoms\\n(M1)', shape = ellipse, style = filled, fillcolor = lightyellow]
  M2 [label = 'Physical\\nActivity\\n(M2)', shape = ellipse, style = filled, fillcolor = lightyellow]
  Y  [label = 'Functional\\nDependence\\n(Y)', shape = ellipse, style = filled, fillcolor = lightblue]

  # Paths
  X  -> M1 [label = 'a1: 0.178']
  X  -> M2 [label = 'a2: -0.042']
  M1 -> M2 [label = 'a3: -0.030']
  M1 -> Y  [label = 'b1: 0.144']
  M2 -> Y  [label = 'b2: -0.203']

  # Direct effect (c′)
  X  -> Y  [label = 'c′: 0.124', color = red, fontcolor = red]

  # Total effect (c)
  X  -> Y  [label = 'c: 0.159', style = dashed, color = gray, fontcolor = gray, constraint = false]

  # Indirect effect summary note
  Note [label = 'Indirect effects\\nind1: 0.026\\nind2: 0.008\\nind3: 0.001\\nTotal indirect: 0.035',
        shape = note, fillcolor = gray95, style = filled]
  X -> Note [style = invis]
...
## Illustration of the mediation model based on the mediation Package
grViz("
digraph mediation_model {

  graph [layout = dot, rankdir = LR, splines = true, ranksep = 1.2]
  ...
  M1 -> Y [label = 'b1\\nACME = 0.0028']
  M2 -> Y [label = 'b2\\nACME = 0.0012']
  X -> Y  [label = 'c\u2032 = 0.0131', color = red]
  
  # Total effect (separate arrow, dashed, from X to Y)
  X -> Y  [label = 'c = 0.0159', style = dashed, color = gray, fontcolor = gray, constraint = false]
